...
# 结束界面显示
def show_end_interface(screen, width, height, is_win, current_sound):
    bg_img = pygame.image.load("images/others/background.png")
    screen.blit(bg_img, (0, 0))
    if is_win:
        # 胜利音乐
        music_win_sound = pygame.mixer.Sound("audios/win.wav")
        music_win_sound.set_volume(1)
        pygame.mixer.Sound.stop(current_sound)
        current_sound = music_win_sound
        current_sound.play()

        font = pygame.font.Font('font/simkai.ttf', width // 14)
        content = font.render(u'你成功守卫了特朗普总统!', True, (255, 0, 0))
        rect = content.get_rect()
        rect.midtop = (width / 2, height / 2)
        screen.blit(content, rect)
    else:
        # 失败音乐
        music_lose_sound = pygame.mixer.Sound("audios/fail.wav")
        music_lose_sound.set_volume(0.5)
        pygame.mixer.Sound.stop(current_sound)
        current_sound = music_lose_sound
        current_sound.play()

        font = pygame.font.Font('font/simkai.ttf', width // 18)
        content = font.render(u'特朗普总统被刺杀，美国沦陷了！', True, (255, 0, 0))
        rect = content.get_rect()
        rect.midtop = (width / 2, height / 2)
        screen.blit(content, rect)
    pygame.display.update()
...
def show_switch_stage(screen, width, height, stage):
    ...
    content = font.render(u'第%d关' % stage, True, (255, 0, 0))
    rect = content.get_rect()
    rect.midtop = (width / 2, height / 2)
    screen.blit(content, rect)
    pygame.display.update()
...
def main():
    ...
    bang_sound.set_volume(1.6)
    d1 = pygame.mixer.Sound("audios/dead1.wav")
    d1.set_volume(18)
    d2 = pygame.mixer.Sound("audios/dead2.wav")
    d2.set_volume(18)
    d3 = pygame.mixer.Sound("audios/dead3.wav")
    d3.set_volume(18)
    md1 = pygame.mixer.Sound("audios/me_dead1.wav")
    md1.set_volume(18)
    md2 = pygame.mixer.Sound("audios/me_dead2.wav")
    md2.set_volume(18)
    md3 = pygame.mixer.Sound("audios/me_dead3.wav")
    md3.set_volume(18)

    # 音乐盒加载
    music_1_sound = pygame.mixer.Sound("audios/1.wav")
    music_1_sound.set_volume(1)
    music_2_sound = pygame.mixer.Sound("audios/2.wav")
    music_2_sound.set_volume(1)
    music_3_sound = pygame.mixer.Sound("audios/3.wav")
    music_3_sound.set_volume(1)
    music_4_sound = pygame.mixer.Sound("audios/4.wav")
    music_4_sound.set_volume(1)

    # 道具音效
    add_sound = pygame.mixer.Sound("audios/add.wav")
    add_sound.set_volume(1)
    time_sound = pygame.mixer.Sound("audios/time.wav")
    ...
    while not is_gameover:
        ...
        enemytanks_now = 0
        # 场上可以存在的敌方坦克总数量
        enemytanks_now_max = min(max(stage * 2, 6), 12)
        # 精灵组
        ...
        while True:
            ...
            if key_pressed[pygame.K_w]:
                ...
                tanksGroup.add(tank_player1)
                player1_moving = True
            elif key_pressed[pygame.K_j]and (bullet_amount > 0):
            ...
            if key_pressed[pygame.K_0]:
                pygame.mixer.Sound.stop(current_sound)
                current_sound = music_1_sound
                current_sound.play()
            elif key_pressed[pygame.K_9]:
                pygame.mixer.Sound.stop(current_sound)
                current_sound = music_2_sound
                current_sound.play()
            elif key_pressed[pygame.K_8]:
                pygame.mixer.Sound.stop(current_sound)
                current_sound = music_3_sound
                current_sound.play()
            elif key_pressed[pygame.K_7]:
                pygame.mixer.Sound.stop(current_sound)
                current_sound = music_4_sound
                current_sound.play()

            # 换皮肤
            if key_pressed[pygame.K_p]:
                tank_player.change_skin()
            elif key_pressed[pygame.K_o]:
                tank_player.change_skin_back()
                    ...
                    for each in enemytanksGroup:
                        if each.being:
                            if pygame.sprite.collide_rect(tank_player.bullet, each):
                                ...
                                if each.blood < 0:  # 随机击杀音效
                                    dead = random.randint(1, 3)
                                    if dead == 1:
                                        sound_dead = d1
                                        sound_dead.play()
                                    elif dead == 2:
            ...
            clock.tick(60)
    if not is_gameover:
        show_end_interface(screen, 630, 630, True, current_sound)
    else:
        show_end_interface(screen, 630, 630, False, current_sound)
